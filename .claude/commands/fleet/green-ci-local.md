---
description: Drive a repo's CI to green LOCALLY with Local CI (Docker) - run a workflow in containers, fix the first paused failure, retry in place, loop until green. The local pre-flight before a push or a remote build-matrix dispatch.
---

Run `$ARGUMENTS` through Local CI locally and drive it to green without a push or
...
is the eyes-only driver (launch Local CI `--pause-on-failure`, classify the
