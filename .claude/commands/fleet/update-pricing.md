...
Use as a phase of the weekly `updating` umbrella, on demand when prices move, or when `check --all` warns a service's pricing snapshot is stale. Exits without writing if neither the vendor page nor the feed yields a price - a stale real snapshot beats a guessed price.
In the wheelhouse, edit `template/base/universal/` and cascade - the live copies are cascade-derived.
