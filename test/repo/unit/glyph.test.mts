...
test('cat and workflow nodes retain independent mask behavior', () => {
  expect(REPO_GLYPH.parts).toHaveLength(2)
  expect(REPO_GLYPH.parts[0]!.cutouts!.length).toBeGreaterThan(0)
  expect(REPO_GLYPH.parts[1]).not.toHaveProperty('cutouts')
})
